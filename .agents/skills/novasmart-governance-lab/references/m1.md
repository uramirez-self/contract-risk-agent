...
| The promo agent's read is **actually denied** | the `status.code=7` denial query (§5) — quote the entry. Absent after a retry → **not verified**, never ✅ | the entry you read, with its gate and resource (§4 Step 5), or **not verified** |
